...
\item \textbf{SWE-bench-lite null result}: Section~\ref{sec:swebench-phase2} reports zero resolved instances across baseline, organism, and LangGraph on 10 SWE-bench-lite instances with Gemma~4. This is a ceiling imposed by model capability, not by the harness --- but it means the harness's transfer value on hard mechanistic tasks is demonstrated by \emph{preservation} (Section~\ref{sec:preservation}) and \emph{primitive reuse} (the escalation experiment), not by independent task-resolution gains over direct prompting at small model scale.
\end{itemize}
