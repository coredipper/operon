...
independent detection reaches.  Bayesian stagnation detection is an
honest negative: the naive cosine-similarity detector outperforms the
two-signal biological design on detection accuracy with mock embeddings,
though at the cost of a 75\% false-positive rate.  The pattern across
all three benchmarks: biological design earns its complexity through
\emph{mechanism-level structural guarantees}---priority gating, signal
accumulation with temporal decay---rather than through algorithmic
sophistication.
\end{abstract}
