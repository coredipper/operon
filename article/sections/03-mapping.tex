...
Finally, we address the physical constraints of computation. Just as biological systems are limited by ATP availability \cite{lynch2015bioenergetic}, agentic systems are limited by token budgets and latency. To model this, we extend the agentic coalgebra to be a \textbf{Metabolic Coalgebra}.
...
For any agentic topology $T$ composed of $N$ agents with total budget $R_{total}$, the system is guaranteed to halt. The ``Halting Problem'' is decidable for Metabolic Coalgebras because the resource state $r$ is strictly decreasing for every non-identity morphism \cite{boreale2023weighted}.
\end{theorem}
