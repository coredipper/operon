...
The polynomial functor formalism applies at every level of biological and software organization. Definitions~1 and~2 establish the \textit{atomic} isomorphism: a single gene and a single agent capability share the same interface structure. But polynomial functors compose---via the parallel ($\otimes$), serial ($\circ$), and trace ($\mathrm{Tr}$) operations of \S\ref{sec:operad}---yielding the same mathematical object at each successive scale:
...
\midrule
Gene & Capability (tool / behavior) & Atomic polynomial functor\\
Cell & Agent & Composite of genes $+$ organelles\\
Tissue & Multi-agent system & Wiring diagram of cells (\S\ref{sec:multi-cellular})\\
\bottomrule
\end{tabular}
\end{center}

\noindent A cell is not merely a bag of genes; it is a structured composition with shared infrastructure (organelles) and a boundary (membrane). Likewise, an agent is a structured composition of capabilities with shared components (LLM provider, memory, error handling) and a security boundary. The organelle mappings below describe this cell-level architecture. The multi-cellular organization of \S\ref{sec:multi-cellular} then composes cells into tissues via wiring diagrams---the same formalism, one level up.
...
Two state machines $(S_1, \alpha_1)$ and $(S_2, \alpha_2)$ are \textbf{bisimilar} if, for every input sequence, they produce identical output sequences. We write $M_1 \sim M_2$ when
\begin{equation}
\forall \, \mathbf{i} \in I^*: \quad \mathrm{readout}_1^*(\mathbf{i}) = \mathrm{readout}_2^*(\mathbf{i})
\label{eq:bisimulation}
\end{equation}
where $\mathrm{readout}^*$ denotes the lifted readout over the input sequence. A diverging input (witness) constitutes a proof of non-equivalence. This enables formal verification: two agent implementations are interchangeable if they are bisimilar, regardless of their internal state representations.
...
Let $(\mathcal{R}, +, 0, \ge)$ be an ordered commutative monoid representing computational resources. For a single resource dimension (e.g., token counts), $\mathcal{R} \cong \mathbb{N}$; for multi-dimensional accounting (latency, memory), $\mathcal{R} \cong \mathbb{R}_{\ge 0}^k$.
\end{definition}
...
Assume a resource-cost function assigns each non-identity transition a cost $c \ge c_{\min} > 0$, the resource
state is monotone nonincreasing (no regeneration), and identity morphisms are the only zero-cost transitions.
Then for any agentic topology with total budget $R_{total}$, every execution has length at most
$\lfloor R_{total} / c_{\min} \rfloor$, so termination is decidable. If regeneration or zero-cost cycles are
permitted, termination requires an additional well-founded potential or an explicit budget/termination
certificate \cite{boreale2023weighted}.
\end{theorem}

\subsection{Additional Organelles: Completing the Cellular Architecture}

Beyond the core interface mapping, biological cells contain specialized organelles that handle distinct aspects of cellular function. We extend our isomorphism to four additional structures that map directly to agentic software components.
