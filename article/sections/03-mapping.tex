...
\paragraph{Definition 2 (The Agent-Capability Object).}
...
The polynomial functor formalism applies at every level of biological and software organization. Definitions~1 and~2 establish the \textit{atomic} correspondence: a single gene and a single agent capability share the same interface form. Polynomial functors then compose---via the parallel ($\otimes$), serial ($\circ$), and trace ($\mathrm{Tr}$) operations of \S\ref{sec:operad}---allowing the same interface language to describe progressively richer assemblies:
...
By establishing this formal dictionary (Table~1), we can compare selected GRN components and agentic
...
\bottomrule
\end{tabular}
\caption{The Correspondence Dictionary (Extended)}
\end{table}
