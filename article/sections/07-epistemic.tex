...
\item[(c)] \textbf{Epistemic ceiling:} If subtasks have unresolved dependencies---$I(\varphi_i; \varphi_j \mid \mathrm{obs}_i) > 0$, meaning agent $i$'s observations do not screen off the dependence on $\varphi_j$---then parallel execution necessarily sacrifices solution quality.
\end{enumerate}
\end{theorem}
...
\textbf{Part (c).} The assumption $I(\varphi_i; \varphi_j \mid \mathrm{obs}_i) > 0$ means there exist global states $s, s'$ with $\mathrm{obs}_i(s) = \mathrm{obs}_i(s')$ (agent $i$ cannot distinguish them) but where the optimal action for subtask $i$ differs: $f_i^*(s) \ne f_i^*(s')$, because the states differ in $\varphi_j$ which influences $\varphi_i$'s optimal answer and this influence is not captured by $\mathrm{obs}_i$. Under independent execution, agent $i$ must choose a single action for both $s$ and $s'$, so it is necessarily suboptimal in at least one case. The expected quality loss is strictly positive whenever such unresolvable states have nonzero probability.
\end{proof}
