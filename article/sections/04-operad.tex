...
\paragraph{Proof.}
The injection can only enter through an input channel (user input, tool output, retrieval, or internal state). Each channel has a fixed provenance assignment in the wiring diagram. Content-based attacks cannot elevate provenance because $\mathcal{P}$ is a functor on the \textit{structure} of the message flow, not its content. The only way to obtain a higher-trust label is to compromise the trusted channel itself (e.g., a malicious tool), which is outside the agent topology.
