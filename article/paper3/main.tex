...
\bottomrule
\end{tabular}
\caption{Phase~A: config-only evolution (40 assessments, 4 tasks, 10
iterations). Rich filesystem context improves the LLM proposer $3\times$
over compressed history but only marginally exceeds tournament mutation.}
\end{table}
...
\bottomrule
\end{tabular}
\caption{Phase~A (config-only) vs Phase~B (topology + config). Tournament
\emph{improves} with topology mutations (+0.16); the LLM proposer
\emph{degrades} ($-$0.13). Blind mutation handles structural changes more
