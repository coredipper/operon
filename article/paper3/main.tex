...
\midrule
Tournament & --- & 0.44 & 9 \\
LLM & compressed (index only) & 0.15 & 2 \\
LLM & rich (configs + traces) & 0.49 & 24 \\
\bottomrule
\end{tabular}
\caption{Phase~A: config-only evolution (4 tasks, 10 iterations).
Tournament runs before stall threshold triggers LLM; remaining
assessments are LLM-proposed (plus fallbacks not shown). Rich context
improves the LLM proposer $3\times$ over compressed history but only
marginally exceeds tournament.}
\end{table}
