...
\midrule
Tournament & 0.44 & 9 \\
LLM (successful parse) & 0.49 & 24 \\
LLM (parse failed, tournament fallback) & 0.41 & 7 \\
\midrule
\emph{Overall LLM path} & \emph{0.47} & \emph{31} \\
\bottomrule
\end{tabular}
\caption{Phase~A definitive run: config-only evolution with rich
filesystem context (4 tasks, 10 iterations, 40 assessments). Tournament
runs first (9 steps before stall threshold), then the LLM proposer
dominates. In a separate earlier run with compressed context (index
entries only), the LLM scored $0.15$ (n=2)---a $3\times$ degradation
from removing filesystem history.}
\end{table}
