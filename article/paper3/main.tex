...
\begin{tabular}{llcc}
\toprule
\textbf{Proposer} & \textbf{Context} & \textbf{Mean Score} & \textbf{N} \\
\midrule
Tournament & --- & 0.44 & 9 \\
LLM (success) & compressed (index only) & 0.15 & 2 \\
LLM (success) & rich (configs + traces) & 0.49 & 24 \\
LLM (fallback)\footnote{LLM proposer fired but response parsing failed; tournament mutation used instead.} & rich & 0.41 & 7 \\
\bottomrule
\end{tabular}
\caption{Phase~A: config-only evolution (4 tasks, 10 iterations, 40
assessments total). Tournament runs before stall threshold triggers LLM.
Rich context improves successful LLM proposals $3\times$ over compressed
history. Including fallbacks, the overall LLM path averages $0.46$.}
\end{table}
...
LLM & 0.49 (n=24) & 0.36 (n=30) & $-$0.13 \\
\bottomrule
\end{tabular}
\caption{Phase~A (config-only) vs Phase~B (topology + config).
Tournament N is consistent (first 9 steps before stall in both phases).
Tournament \emph{improves} with topology mutations; the LLM proposer
\emph{degrades}. Blind mutation handles structural changes more
productively than reasoned proposals.}
\end{table}
