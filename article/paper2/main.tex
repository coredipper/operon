...
\large A Typed Adapter Architecture for Multi-Agent System Integration\\[0.5em]
...
We present a typed adapter architecture centred on a single intermediate
type, \texttt{ExternalTopology}, that lets Operon's structural analysis layer
analyze \emph{any} external agent topology without importing the target
framework.  Five adapters parse framework-specific configurations into this
...
Our contribution is a \emph{typed adapter architecture} that decouples
structural analysis from runtime execution:

\begin{enumerate}[leftmargin=*,itemsep=2pt]
  \item \textbf{ExternalTopology} as a universal intermediate type:
    five adapters parse five frameworks into a single frozen dataclass;
    all downstream analysis is source-agnostic.
...
The composite risk score is a weighted sum:
\[
  r \;=\; w_1 P_{\mathrm{error}}(n) \;+\; w_2 \rho_{\mathrm{seq}}
    \;+\; w_3 (1 - S^{-1}) \;+\; w_4 \min(\delta / 5, 1)
\]
with default weights $w_1 = 0.4$, $w_2 = 0.25$, $w_3 = 0.15$,
$w_4 = 0.2$.
...
parameters (2--3 organisms, 2--3 templates/tools, bounded iteration
...
Models cross-organism template sharing with trust-weighted adoption,
inspired by horizontal gene transfer with epistemic vigilance.  An organism
imports a peer's template only when three guards are satisfied: (i) the
organism's developmental stage meets the template's minimum stage,
...
Organisms consume telomere capacity, advancing through four stages
(Embryonic $\to$ Juvenile $\to$ Adolescent $\to$ Mature).  The key
safety property is \emph{critical period irreversibility}: once a period
transitions from ``open'' to ``closed,'' it can never reopen.  This
models neurodevelopmental critical periods where specific circuits are
maximally plastic only during a bounded window.

\paragraph{ConvergenceDetection.}
Models the watcher's convergence protocol.  As organisms execute stages,
...
organism is halted in the same atomic step---a safety property expressing
...
progression: the organism never regresses.  The specification intentionally
...
Following Zardini's co-design theory~\cite{zardini2023codesign}, we model
each adapter as a \emph{design problem} (DP)---a monotone map from a
resource poset to a functionality poset.  Monotonicity means that more
resources yield at least as many functionalities.
...
\begin{theorem}[Adaptive Loop Convergence]
Let $\mathsf{DP}$ be the feedback composition of the adapter stack, with
scoring function $s: \mathcal{F} \to [0, 1]$ and feedback map
$\phi(r, f) = r \cup \{s(f)\}$.  If $s$ is monotone and the resource
poset is finite, then the iterative sequence
$r_0, r_1 = \phi(r_0, f(r_0)), r_2 = \phi(r_1, f(r_1)), \ldots$
converges to a fixed point $r^*$ within finitely many iterations such
that $|s(f(r^*)) - s(f(r_{n-1}))| < \varepsilon$ for any
$\varepsilon > 0$.
\end{theorem}

\begin{proof}
The scoring function $s$ maps functionalities to $[0, 1]$, a complete
lattice under the standard order.  The feedback map $\phi$ is monotone by
composition of monotone maps ($f$ is monotone by the DP definition; $s$ is
monotone by assumption).  By Kleene's fixed-point theorem applied to
monotone endomorphisms on a complete lattice, the ascending chain
$s_0 \leq s_1 \leq s_2 \leq \cdots$ converges to $\sup_n s_n = s^*$.
Since $[0, 1]$ is bounded, the chain stabilizes: there exists $N$ such
that $s_N = s^*$.  In practice, the implementation terminates when
$|s_{n+1} - s_n| < \varepsilon$ for a configurable $\varepsilon$ (default
$0.01$), with a safety bound of 100 iterations.
\end{proof}
...
(fallback), an LLM generator produces a one-shot organism, which is
...
effects.  Our work provides the formal infrastructure---typed composition,
...
compile Operon organisms into Swarms workflow configurations or DeerFlow
...
We have presented a typed adapter architecture for integrating Operon's
...
architecture achieves framework-agnostic analysis while preserving full
type safety.  The 14 convergence modules, 216 convergence tests, 4 TLA+
