...
Then for any topology with finite budget $R_{total}$, every execution has length at most
$\lfloor R_{total} / c_{\min} \rfloor$, so termination is decidable. If regeneration or zero-cost cycles are
permitted, termination requires an additional well-founded potential or explicit budget/termination certificate.
\end{theorem}
