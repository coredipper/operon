...
def test_call_arity_no_args():
    def f(): return "none"
    assert call_arity(f, 1, 2, 3) == "none"

def test_call_arity_positional():
    def f1(a): return a
    def f2(a, b): return a + b

    assert call_arity(f1, 1, 2, 3) == 1
    assert call_arity(f2, 1, 2, 3) == 3

def test_call_arity_var_args():
    def f(*args): return args
    assert call_arity(f, 1, 2, 3) == (1, 2, 3)

def test_call_arity_var_kwargs():
    def f(**kwargs): return kwargs
    # If a function only takes **kwargs, call_arity currently passes all args because VAR_KEYWORD is present.
