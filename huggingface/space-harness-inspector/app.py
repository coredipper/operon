...
def inspect_architecture(
    ...
    # --- Architecture Triple ---
    # G (graph)
    edges_html = f' <span aria-hidden="true">&rarr;</span><span style="{SR_ONLY}"> to </span> '.join(_esc(s) for s in arch.stage_names)
    g_content = (
        f'<div style="font-family:monospace;font-size:1.1em;margin-bottom:8px;">'
        f'{edges_html}</div>'
        f'<table style="width:100%;border-collapse:collapse;">'
        f'<tr style="border-bottom:1px solid #e5e7eb;">'
        f'<th scope="row" style="padding:4px 8px;color:#6b7280;text-align:left;font-weight:normal;">Stages</th>'
        f'<td style="padding:4px 8px;">{[_esc(s) for s in arch.stage_names]}</td></tr>'
        f'<tr style="border-bottom:1px solid #e5e7eb;">'
        f'<th scope="row" style="padding:4px 8px;color:#6b7280;text-align:left;font-weight:normal;">Edges</th>'
        f'<td style="padding:4px 8px;">{[(_esc(a), _esc(b)) for a, b in arch.edges]}</td></tr>'
        f'<tr>'
        f'<th scope="row" style="padding:4px 8px;color:#6b7280;text-align:left;font-weight:normal;">Sequential</th>'
    ...
    tier_map = {
        "fixed": "fast",
        "fuzzy": f'fast<span aria-hidden="true">&rarr;</span><span style="{SR_ONLY}"> to </span>deep',
        "deep": "deep",
    }
    phi_rows = ""
    ...
    arch_html = (
        ...
        + _section(f'&Phi; <span aria-hidden="true">&mdash;</span> Interface (mode <span aria-hidden="true">&rarr;</span><span style="{SR_ONLY}"> to </span> model mapping)', phi_content, "#a855f7")
    )
