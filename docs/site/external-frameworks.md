...
**Update (2026-05-26):** §8.8 added — *The Log is the Agent* (Nakajima, [arXiv:2605.21997](https://arxiv.org/abs/2605.21997), bib key `nakajima2026activegraph`) frames ActiveGraph as an event-sourced **alternative L1 substrate**, sibling of LangGraph itself rather than of the adapter family. Recorded as a *deferred-by-discipline* attach-point candidate: ActiveGraph's `@behavior()` / `@relation_behavior()` decorators are natural gate attach points, but no adapter ships until a pull trigger fires (per the §8.4 v0.2 scope decision). The entry strengthens the portability claim by adding a fourth substrate the gates could attach to (LangGraph in-tree, gascity §8.1, agentflow §8.3, ActiveGraph deferred) without growing operon's API surface. No code shipped.

**Update (2026-05-27):** §8.9 added — `iii-hq/iii` ([iii.dev](https://iii.dev/), [GitHub](https://github.com/iii-hq/iii)) is the first §8 entry spanning languages — a Rust engine with Python/Node/Rust SDKs orchestrating Workers, Functions, and Triggers across processes. Recorded as a *deferred-by-discipline* attach-point candidate, but the case is **weaker than §8.8** in two specific ways: iii exposes no decorator or hook surface (only the `trigger()` invocation boundary + built-in OpenTelemetry spans), so a bridge would have to wrap an invocation rather than compose with a hook; and the multi-language footprint means a Python-only adapter would cover one of three SDK surfaces, leaving the portability claim asymmetric. The entry adds a new dimension to the substrate-portability story (every prior §8.x entry is single-language) without growing operon's API surface. No code shipped, per §8.4 v0.2 pull-gate discipline.
...
### 8.9 iii — multi-language L1 substrate, deferred-by-discipline

`iii-hq/iii` ([iii.dev](https://iii.dev/), [GitHub](https://github.com/iii-hq/iii); 16.6k stars, commit cadence current through 2026-05-27) is a distributed orchestration runtime built around three primitives: **Workers** (service instances), **Functions** (registered capabilities), and **Triggers** (activation mechanisms — HTTP, cron, queue, stream, state-change, custom). A Rust engine routes invocations across workers that self-register via WebSocket and discover each other's functions live; SDKs ship in Python (PyPI), Node (npm), and Rust (crates.io). Licensing is split: Elastic License 2.0 on the engine, Apache 2.0 on the SDKs — production-viable on the SDK side but source-available rather than strictly open-source on the engine. The headline pitch is the explicit thesis that *"the complexity of actual integrations is quadratic — overwhelming for devs and for AI context. iii fundamentally eliminates this complexity."* Built-in observability ships as OpenTelemetry traces/spans/baggage that propagate across invocation boundaries automatically.

**Placement.** L1 *substrate* — sibling of the §1 L1 row (Swarms / DeerFlow / gascity / agentflow / LangGraph / ActiveGraph §8.8), and the **first §8 entry spanning languages**. Every prior §8.x entry is single-language by construction (gascity Go, agentflow Python, ActiveGraph Python, LangGraph Python); iii is the first substrate the survey has recorded that publishes peer SDKs in three runtimes over a shared engine. That cross-language footprint is the load-bearing novelty for the portability story the gates wedge depends on — if the *certificate-as-replayable-evidence* contract holds across language boundaries, it claims a stronger generality than any single-language substrate can supply. The natural attach-point candidates are the `trigger()` invocation boundary (the only function-invocation seam exposed at the SDK level) and the built-in OpenTelemetry span emission (a parallel diagnostic channel, akin to §8.4 *veloryn-xray*'s observational-only territory but native rather than bolted on). There is **no decorator or hook surface** analogous to ActiveGraph's `@behavior()` / `@relation_behavior()` (§8.8), no policy/validator extension protocol, and no compile-time graph the way agentflow's `Graph()` context manager (§8.3) supplies — security is implicit (WebSocket auth to the engine, no first-class access-control extensibility).

**Verdict — multi-language attach-point candidate, no adapter scheduled.** The portability-story arithmetic that made §8.8 ActiveGraph worth recording applies here too, with one strengthening qualification: iii adds a *language* dimension on top of the *substrate* dimension. Through the Ning et al. *Code as Agent Harness* survey lens (arXiv:2605.18747, bib key `ning2026codeharness`), iii delivers §5.2.1 metric dimensions **(i) trajectory efficiency** (via cross-worker routing) and **(vi) replayability** (via OTel span correlation), but solidly **misses §5.2.2** — there is no scope-typed certificate, no declarative "this verifies X but not Y" surface, no machine-checked binding spec. The §5.2.2 gap is the same one that motivated the operon-langgraph-gates v0.2 dogfood contract (§8.4 *v0.2 execution*); iii's case for that gap is structurally weaker than §8.8's because iii does not even expose a decorator surface where a verifier could attach — a bridge would have to wrap `iii.trigger()` at the SDK layer, intercepting invocations rather than slotting into a hook.

Per the v0.2 scope discipline (the 2026-05-19 decision recorded in §8.4 above), **no `IiiCertificateAdapter` ships until a pull trigger fires**. The pull-gate is the same as for §8.8: a first external issue, a sibling adapter that genuinely needs cross-language reach, or measured download/clone inflection on the existing operon-langgraph-gates surface. A second consideration specific to iii: a Python-only adapter would cover only one of three SDK runtimes, leaving the portability claim asymmetric — a *language*-portable wedge would imply Python + Node + Rust adapters in parity, which is firmly outside any "harden + dogfood contract" v0.2 envelope. Honest position: iii is the survey's first multi-language datum and meaningfully strengthens the portability claim by its mere existence at L1; the adapter waits, the SDK-fan-out question waits longer.
