...
## 2024-05-18 - Replacing time.sleep with Event.wait
**Learning:** Using time.sleep() in long-running background threads causes unnecessary blocking and delays thread shutdown latency because the thread cannot instantly interrupt its sleep if a stop signal is received.
**Action:** Replace synchronous time.sleep(timeout) calls inside threading.Thread loops with threading.Event().wait(timeout) (e.g., self._stop_event.wait(timeout)) to achieve non-blocking delay and instant exit upon shutdown.
