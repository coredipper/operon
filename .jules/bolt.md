...
## $(date +%Y-%m-%d) - Thread Sleep Latency Optimization
**Learning:** When implementing periodic loops in background threads, using `time.sleep(timeout)` causes the thread to block synchronously. When stopping the thread, this introduces an artificial delay (latency) up to the sleep duration before the thread can check its stop condition and exit.
**Action:** Replace `time.sleep(timeout)` with `self._stop_event.wait(timeout)` (using a `threading.Event`). This still prevents busy waiting by sleeping for the timeout duration, but allows the thread to immediately interrupt its sleep and exit if the stop event is set, reducing shutdown latency to near-zero.
