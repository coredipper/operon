...
## 2026-05-27 - Pre-compiling Regex in Wire-Level Filters
**Learning:** In the `operon_ai` project, data transformation filters (like `StripMarkupFilter` in `DenatureFilter`) applied regex operations (e.g. `re.sub(r"\n{3,}", "\n\n", result)`) inline during each method call. Pre-compiling these regular expressions at the class level via `typing.ClassVar` reduces execution overhead by avoiding dictionary cache lookups, providing a measurable performance benefit in frequently called data-cleaning hot paths.
**Action:** Always verify if regex operations inside frequently executed data parsing or filtering methods are defined as inline strings instead of pre-compiled `re.Pattern` objects. Pre-compile them at the class level with `ClassVar` to ensure maximum performance without polluting dataclass constructors.
