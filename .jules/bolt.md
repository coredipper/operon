...
## 2026-05-14 - Pre-compiled Tuple vs Set Membership Evaluation
**Learning:** In Python, using a `frozenset` or `set` for testing membership is marginally faster (~10%) than using a `tuple` or `list` for very small constant sizes (like 3 elements), but initializing them is slower. More importantly, pre-compiling a `tuple` at the module or class level yields substantial performance gains (about 3x to 5x faster than inline lists or inline enums) and has the lowest memory overhead. The real bottleneck to avoid is inline creation of structures (`return val in ('a', 'b', 'c')` vs `return val in ['a', 'b', 'c']` which are fast, compared to `return val in [Enum.A, Enum.B]`). Specifically, building lists inline is what causes the slowdown.
**Action:** When performing membership checks against a static list of strings or enums inside frequently called methods or properties (like `is_healthy` or `is_success`), extract the collection to a module-level or class-level `ClassVar` tuple. This prevents the tuple/list and the enum attribute lookups from being repeatedly constructed.

## 2026-05-14 - Lazy Evaluation with regex.finditer over findall
**Learning:** For situations where we want to process all matches of a regex without needing to store them in a single indexable list, `re.finditer` with a generator expression is more memory-efficient and slightly faster than `re.findall`. In loops like `vocabulary.update(words)`, `re.finditer` avoids the overhead of explicitly allocating a large list of string matches.
**Action:** Replace `re.findall` with `re.finditer` and a generator expression when the matches are only used for iteration (like adding to a set or writing to a file), particularly when processing potentially large blocks of text.
